...
def test_historic_5day_temperature_returns_24_reasonable_float_values_for_each_day():
    historic_5days_and_today_results = get_all_hist_temp((55.751244, 37.618423))
    historic_5days_results = list(historic_5days_and_today_results)[:5]
    for result in historic_5days_results:
        assert len(result) == 24
        assert all(abs(float(result[i])) < 70 for i in range(24))
...
def test_forecast_temperature_func_returns_8_reasonable_float_values_for_each_of_4_coming_days():
    ...
    for day_temps in forecast_4days:
        ...
        assert all(abs(float(day_temps[i])) < 70 for i in range(8))
